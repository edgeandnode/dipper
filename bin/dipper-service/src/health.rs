...
//!
//! The supervisor ([`crate::supervisor`]) catches a worker that *exits*
//! (panic or error return). It cannot catch a worker that is *wedged* — alive
//! but making no progress (e.g. parked on an await that the per-call timeouts
//! somehow didn't bound). This module closes that gap: the worker ticks a
//! progress watermark every loop iteration, and a small health server reports
//! 503 once the watermark goes stale so an external orchestrator (k8s liveness
//! probe) can restart the wedged process.
...
///
/// Must exceed the worker's worst-case time between progress ticks: the poll
/// period plus a single job's backstop timeout
/// ([`crate::worker::service`]'s `PROCESS_JOB_TIMEOUT`, 300s). 600s leaves
/// comfortable headroom so a legitimately slow job never trips the probe.
...
mod tests {
    ...
    async fn stalled_client_does_not_block_other_probes() {
        ...
        // stalled client (well under CONN_TIMEOUT).
