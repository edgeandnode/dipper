...
def derive_timestamps(num_days):
    ...
    end_date = today
    start_date = today - timedelta(days=self.num_days)
...
def perform_linear_regression(df):
    ...
    categorical = ['indexer', 'deployment_hash', 'indexer_network', 'sampled_query_id_hashed_mod_integer_root']
...
def aggregate_indexer_info(df):
    ...
    """
    agg_df = df.groupby('indexer').agg({
        'org': lambda x: x.mode()[0],
    ...
    agg_df['destination_loc'] = agg_df['destination_loc'].apply(lambda x: process_location(x) if pd.notna(x) else x)
