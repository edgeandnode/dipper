...
/// This function needs to be called from within a pyo3 context as early as possible to ensure
        ...
        indoc::formatdoc! {r#"
            ...
            def basicConfig(*pargs, **kwargs):
                if "handlers" not in kwargs:
                    kwargs["handlers"] = [RustLogHandler()]

                return _basicConfig(*pargs, **kwargs)
            "#,
            target=target
        }
        .as_str(),
